...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (strcmp(acao, "II") == 0) {
            int num;
            scanf("%d", &num);
            inserirInicio(*pokemon[num-1]);
            
        } else if (strcmp(acao, "I*") == 0) {
            scanf("%d %d", &pos, &num);
            if (num >= 1 && num <= n && pos >= 0) {
                inserir(*pokemon[num-1], pos);
            } else {
    ...
    free(input);
    liberarLista(); // Libera a lista de Pokémon
