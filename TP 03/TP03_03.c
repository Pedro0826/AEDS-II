...
void inserirFim(Pilha *pilha, Pokemon x) {
    if (pilha->n >= 801) {
        printf("Erro ao Inserir!\n");
        exit(1);
    }

    pilha->pokemons[pilha->n] = malloc(sizeof(Pokemon));  // Aloca memória para um novo Pokemon
    *(pilha->pokemons[pilha->n]) = x;  // Copia o conteúdo de x
    pilha->n++;
}
...
Pokemon removerFim(Pilha *pilha) {
    ...
    Pokemon rem = *(pilha->pokemons[pilha->n - 1]);
    free(pilha->pokemons[pilha->n - 1]); // Libera a memória do Pokémon removido
    pilha->n--;
...
int main(int argc, char **argv)
{
    ...
    Pilha *pokemons = (Pilha *)malloc(sizeof(Pilha)); // Array de Pokémon começa vazio.
