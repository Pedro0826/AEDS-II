...
#define M 21 // Tamanho da tabela hash
...
// Função para calcular o valor ASCII do nome
int asciiNome(const char *nome) {
    int soma = 0;
...
No* novoNo(const char *nome) {
    ...
    no->nome = strdup(nome); // Copia o nome
    no->prox = NULL;
    return no;
}

// Função para inserir um elemento na tabela hash
void inserir(Hash *hash, Pokemon *pok) {
    int ascii = asciiNome(pok->name);
    int pos = h(ascii, hash->m);

    No *novo = novoNo(pok->name);

    if (hash->tabela[pos] == NULL) {
        hash->tabela[pos] = novo; // Insere diretamente na posição
    } else {
...
void pesquisar(Hash *hash, const char *nome) {
    int ascii = asciiNome(nome);
    int pos = h(ascii, hash->m);
    int resp = 0;

    No *atual = hash->tabela[pos];
    while (atual != NULL) {
...
int main(int argc, char **argv) {
    ...
    Hash *pokemons = criarHash(M); // A árvore começa vazia
