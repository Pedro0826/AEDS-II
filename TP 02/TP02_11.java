...
public class CountingSort {
        ...
        public static int getMaior(Pokemon[] array, int n) {
            int maior = array[0].getTaxaCaptura();
            for (int i = 1; i < n; i++) {
                if (maior < array[i].getTaxaCaptura()) {
                    maior = array[i].getTaxaCaptura();
                }
            }
            return maior;
        }

        private static void stableSortByName(Pokemon[] array, int n, int nComp, int nMov) {
            for (int i = 0; i < n - 1; i++) {
                for (int j = 0; j < n - 1 - i; j++) {
                    // Comparar taxa de captura e, em caso de empate, o nome
                    if (array[j].getTaxaCaptura() == array[j + 1].getTaxaCaptura() &&
    ...
    public static void main(String[] args) {
        ...
        while(!strcmp2(p, "FIM")){
            ...
            int maior = getMaior(pokemonsSec, a);
            int[] count = new int[maior + 1];
            Pokemon[] ordenado = new Pokemon[a];

            //Inicializar cada posicao do array de contagem 
		    for (int i = 0; i < count.length; count[i] = 0, i++);

            // Contar o número de Pokémon com cada taxa de captura
            for (int i = 0; i < a; count[pokemonsSec[i].getTaxaCaptura()]++, i++);

            // Calcular o número de elementos menores ou iguais a cada taxa de captura
            for (int i = 1; i < count.length; count[i] += count[i - 1], i++);

            // Ordenando os Pokémon
            for (int i = a - 1; i >= 0; i--) {
                int index = pokemonsSec[i].getTaxaCaptura();
                // Adiciona ao array ordenado
                ordenado[count[index] - 1] = pokemonsSec[i];
                count[index]--;
            }

            stableSortByName(ordenado, a, nComp, nMov);

    // Copiando para o array original
    for (int i = 0; i < a; pokemonsSec[i] = ordenado[i], i++);
        ...
        try (PrintStream log = new PrintStream("842843_countingsort.txt")) {
