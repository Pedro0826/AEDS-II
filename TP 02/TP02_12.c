...
void Bolha(Pokemon **array, int n, int* nComp, int* nMov) {
    int i, j;
    for (i = 0; i < n - 1; i++) { // Corrigido de i-- para i++
        for (j = 0; j < n - 1 - i; j++) { // Reduzindo o número de comparações a cada iteração
            (*nComp)++; // Incrementa o contador de comparações
            if (array[j]->id > array[j + 1]->id) {
                trocar(&array[j], &array[j + 1], nMov);
            }
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    FILE *log = fopen("842843_bolha.txt", "w");
