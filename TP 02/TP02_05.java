...
public class ordenacaoSelecao {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < a - 1; i++) {
            ...
            for (int j = i + 1; j < a; j++) {
                String nomeI = pokemonsSec[indice].getNome();
                String nomeJ = pokemonsSec[j].getNome();
        
                // Comparar os nomes letra por letra, lidando com tamanhos diferentes
                int minLength = menor(nomeI.length(), nomeJ.length());
                boolean trocou = false;
        
                for (int k = 0; k < minLength; k++) {
                    if (nomeJ.charAt(k) < nomeI.charAt(k)) {
                        indice = j;
                        trocou = true;
                        k = minLength;
                        nComp++;
                    } else if (nomeJ.charAt(k) > nomeI.charAt(k)) {
                        trocou = true;
                        k = minLength;
                        nComp++;
                    }
                ...
                // Se todos os caracteres forem iguais até o tamanho mínimo, o nome menor (mais curto) vem primeiro
                if (!trocou && nomeJ.length() < nomeI.length()) {
                    indice = j;
                    nComp++;
                }
            }
        
            // Trocar o Pokémon atual com o Pokémon no índice do menor nome encontrado
            Pokemon temp = pokemonsSec[i];
            pokemonsSec[i] = pokemonsSec[indice];
            pokemonsSec[indice] = temp;
            nMov += 3;
        }
        ...
        long tempoExecucao = fim - inicio;
            ...
            try (PrintStream log = new PrintStream("842843_selecao.txt")) {
