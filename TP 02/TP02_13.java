...
class Pokemon{
    ...
    public void setTipo(ArrayList<String> tipo) {this.tipo = tipo != null ? tipo : new ArrayList<>();}
...
public class Mergesort {
        ...
        private static void mergesort(Pokemon[] array, int esq, int dir, int nComp, int nMov) {
            if (esq < dir) {
                int meio = (esq + dir) / 2;
                mergesort(array, esq, meio, nComp, nMov);
                mergesort(array, meio + 1, dir, nComp, nMov);
                intercalar(array, esq, meio, dir, nComp, nMov);
            }
        }
        
        public static void intercalar(Pokemon[] array, int esq, int meio, int dir, int nComp, int nMov) {
            int n1 = meio - esq + 1;
            int n2 = dir - meio;
            ...
            while (i < n1 && j < n2) {
                // Comparar os tipos em ordem alfabética
                ArrayList<String> tipo1 = a1[i].getTipo();  // Pegando o primeiro tipo
                ArrayList<String> tipo2 = a2[j].getTipo();  // Pegando o primeiro tipo

                // Verifique se os tipos não são nulos ou vazios
                String tipo1Primeiro = (tipo1 != null && !tipo1.isEmpty()) ? tipo1.get(0) : "";
                String tipo2Primeiro = (tipo2 != null && !tipo2.isEmpty()) ? tipo2.get(0) : "";

                // Comparar os tipos
                int compareTypes = tipo1Primeiro.compareTo(tipo2Primeiro);
                nComp++;
                
                if (compareTypes < 0 || (compareTypes == 0 && a1[i].getNome().compareTo(a2[j].getNome()) < 0)) {
                    array[k++] = a1[i++];
                    nComp+=2;
                    nMov++;
                } else {
    ...
    public static void main(String[] args) {
        ...
        mergesort(pokemonsSec,0, a-1, nComp, nMov);
        ...
        try (PrintStream log = new PrintStream("842843_mergesort.txt")) {
