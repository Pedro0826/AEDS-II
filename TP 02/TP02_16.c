...
#define LOG "842843_insercaoParcial.txt"
...
static void insercao_parcial(Pokemon **vec, size_t n, size_t k);

// Funções para a implementação do objeto Pokémon.
void ler(Pokemon *restrict p, char *str);
void imprimir(Pokemon *restrict const p);
int compara(Pokemon *restrict const p1, Pokemon *restrict const p2);
...
// Compara um Pokémon com outro a partir da data de captura.
int compara(Pokemon *restrict const p1, Pokemon *restrict const p2)
{
	Date d1 = p1->capture_date, d2 = p2->capture_date;
	int res = d2.y - d1.y;
	res = res ? res : d2.m - d1.m;
	res = res ? res : d2.d - d1.d;
	++num_comparacoes;
	return res ? res : strcmp(p1->name, p2->name);
}
...
#define K 10 // Número de termos a ordenar na ordenação parcial.
...
static void insercao_parcial(Pokemon **vec, size_t n, size_t k)
{
	// Não podemos ordenar mais elementos do que existem no vetor.
	k = (k > n) ? n : k;

	// Parte 1: inserção normal de 0 até k.
	...
	// Parte 2: inserção parcial propriamente dita, de k até n - 1;
