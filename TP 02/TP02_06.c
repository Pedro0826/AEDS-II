...
void selecao_recursiva(Pokemon **pokemon, int n, int inicio,int *nComp, int *nMov) {
    if (inicio >= n - 1) {
        return; // Caso base: se o início ultrapassa o penúltimo elemento
    }

    int menor_idx = inicio; // Assume que o primeiro elemento é o menor

    // Encontra o menor elemento na parte não ordenada
    for (int j = inicio + 1; j < n; j++) {
        if (strcmp(pokemon[j]->name, pokemon[menor_idx]->name) < 0) {
            menor_idx = j; // Atualiza o índice do menor elemento
            (*nComp)++;
        }
    }

    // Troca o menor elemento encontrado com o primeiro elemento não ordenado
    trocar(&pokemon[inicio], &pokemon[menor_idx], nMov);

    // Chama a função recursiva para o restante do array
    selecao_recursiva(pokemon, n, inicio + 1, nComp, nMov);
}
...
int main(int argc, char **argv)
{
    ...
    selecao_recursiva(pokemonSec, a, 0, &nComp, &nMov);
    ...
    FILE *log = fopen("842843_selecaoRecursiva.txt", "w");
