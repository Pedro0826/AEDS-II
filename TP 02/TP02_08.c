...
void insercaoPorCor(Pokemon **array, int n, int cor, int h,int *nComp, int *nMov) {
    for (int i = (h + cor); i < n; i += h) {
        Pokemon *tmp = (Pokemon *) malloc(sizeof(Pokemon));
        *tmp = *array[i];
        int j = i - h;

        // Primeiro verifica a ordenação pelo peso
        (*nComp)++;
        while ((j >= 0) && (array[j]->weight > tmp->weight)) {
            array[j + h] = array[j];
            (*nMov)++;;
            j -= h;
            (*nComp)++;
        ...
        // Se os pesos forem iguais, ordena pelo nome
        if (j >= 0 && array[j]->weight == tmp->weight) {
            (*nComp)++;
            while ((j >= 0) && (array[j]->weight == tmp->weight) && (strcmp(array[j]->name, tmp->name) > 0)) {
                (*nComp)++;
                array[j + h] = array[j];
                 (*nMov)++;
                j -= h;
...
void shellsort(Pokemon **array, int n, int *nComp, int *nMov) {
    int h = 1;

    // Calcula o intervalo inicial de h
    ...
    do {
        ...
        for (int cor = 0; cor < h; cor++) {
            insercaoPorCor(array, n, cor, h, nComp, nMov);
        }
    } while (h > 1);
}
...
int main(int argc, char **argv)
{
    ...
    shellsort(pokemonSec, a, &nComp, &nMov);
    ...
    FILE *log = fopen("842843_shellsort.txt", "w");
