...
int buscaBinaria(Pokemon **pokemonSec, int a, const char *input) {
    int baixo = 0;
    int alto = a - 1;
    ...
    while (baixo <= alto) {
        ...
        // Comparação do nome do Pokémon com a entrada
        int cmp = strcmp(pokemonSec[meio]->name, input);

        if (cmp == 0) {
            return 1; // Pokémon encontrado
        } else if (cmp < 0) {
...
void ordenarPokemons(Pokemon **pokemonSec, int a) {
    for (int i = 0; i < a - 1; i++) {
        int indice = i;
        for (int j = i + 1; j < a; j++) {
            if (strcmp(pokemonSec[j]->name, pokemonSec[indice]->name) < 0) {
                indice = j; // Atualiza o índice do menor nome
            }
        }
        // Troca os Pokémon se necessário
        if (indice != i) {
            Pokemon *temp = pokemonSec[i];
            pokemonSec[i] = pokemonSec[indice];
            pokemonSec[indice] = temp;
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    ordenarPokemons(pokemonSec, a);

clock_gettime(CLOCK_MONOTONIC, &inicio);
    while (getline(&input, &tam, stdin) != -1 && strcmp(input, "FIM\n")) {
            input[strcspn(input, "\n")] = 0;
            if (buscaBinaria(pokemonSec, a, input) == 1) {
                printf("SIM\n"); // Pokémon encontrado
            } else {
...
long tempoExecucao = (fim.tv_sec - inicio.tv_sec) * 1e9 + (fim.tv_nsec - inicio.tv_nsec);
    ...
    free(input);
     ...
     FILE *log = fopen("842843_binaria.txt", "w");
