...
public class QuicksortParcial {
        ...
        private static void quicksort(Pokemon[] pokemonsSec, int esq, int dir, int nComp, int nMov) {
            int i = esq, j = dir;
            Pokemon pivo = pokemonsSec[(dir+esq)/2];
            while (i <= j) {
                while (pokemonsSec[i].getGeracao() < pivo.getGeracao() || 
       (pokemonsSec[i].getGeracao() == pivo.getGeracao() && 
       pokemonsSec[i].getNome().compareTo(pivo.getNome()) < 0)) {
                nComp +=2;
            i++;
        }
        
        while (pokemonsSec[j].getGeracao() > pivo.getGeracao() || 
              (pokemonsSec[j].getGeracao() == pivo.getGeracao() && 
              pokemonsSec[j].getNome().compareTo(pivo.getNome()) > 0)) {
                nComp +=2;
            j--;
        }

        if (i <= j) {
            swap(pokemonsSec, i, j);  // Troca os elementos
            i++;
            j--;
            nMov += 3; // Incrementa movimentações, 2 para a troca
        }
    }

    if (esq < j) quicksort(pokemonsSec, esq, j, nComp, nMov);
    if (i < 10 && i < dir) quicksort(pokemonsSec, i, dir, nComp, nMov); // Corrigido para 'i' em vez de 'esq'

    }

    public static void swap(Pokemon[] pokemonsSec, int a, int b) {
        Pokemon temp = pokemonsSec[a];
        pokemonsSec[a] = pokemonsSec[b];
        pokemonsSec[b] = temp; 
    }
    ...
    public static void main(String[] args) {
        ...
        quicksort(pokemonsSec, 0, a-1, nComp, nMov);
        ...
        try (PrintStream log = new PrintStream("842843_quicksortPar.txt")) {
