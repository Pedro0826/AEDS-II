...
void trocar(Pokemon **a, Pokemon **b, int *nMov) {
    Pokemon *temp = *a;
    *a = *b;
    *b = temp;
    (*nMov) += 3;
...
int comparaPokemon(Pokemon* p1, Pokemon* p2, int* nComp, int* nMov) {
    if (p1->generation != p2->generation) {
        (*nComp)++;
        return p1->generation - p2->generation; // Comparação pela geração
    }
    return strcmp(p1->name, p2->name); // Comparação pelo nome
}

void quicksortRec(Pokemon **array, int esq, int dir, int* nComp, int* nMov) {
    int i = esq, j = dir;
    Pokemon* pivo = array[(dir+esq)/2];
    while (i <= j) {
        // Encontra a posição correta de i e j
        while (comparaPokemon(array[i], pivo, nComp, nMov) < 0) {
            i++;
        }
        while (comparaPokemon(array[j], pivo, nComp, nMov) > 0) {
            j--;
        }
        if (i <= j) {
            trocar(&array[i], &array[j], nMov);
            i++;
            j--;
        }
    }
    if (esq < j)  quicksortRec(array, esq, j, nComp, nMov);
    if (i < dir)  quicksortRec(array, i, dir, nComp, nMov);
}
...
int main(int argc, char **argv)
{
    ...
    Pokemon *pokemonSec[NUM_PK] = { NULL };// Array de Pokémon começa vazio.
    ...
    FILE *log = fopen("842843_quicksort.txt", "w");
